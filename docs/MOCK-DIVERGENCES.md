...
| 15 | Formulas | None: every cell is a value | `getValues` returns formula results; `setValues` writes plain values | Any rule that rewrites rows (sort, clear) replaces formulas in those rows with their values. Open for the owner (DECISIONS "P1: the remaining templates"). | open |
